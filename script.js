...
hideLoading();
 ...
 const current = data.current;
        ...
        const dayPhase = current.weather[0].icon.endsWith('d') ? 'day' : 'night';
        setWeatherTheme(condition, dayPhase);
        ...
        weatherIcon.classList.add('icon-animate');
        setTimeout(() => weatherIcon.classList.remove('icon-animate'), 800);
 ...
 if (!dailyForecasts[dayKey]) {
    ...
    function setWeatherTheme(condition, phase) {
        // Clear theme classes
        bodyEl.className = '';
        appContainer.className = 'weather-app';

        const themeType =
            condition.includes('rain') || condition.includes('drizzle') ? 'rain' :
            condition.includes('snow') ? 'snow' :
            condition.includes('storm') || condition.includes('thunder') ? 'thunderstorm' :
            condition.includes('cloud') ? 'clouds' :
            'clear';

        appContainer.classList.add(`theme-${themeType}`);
        appContainer.classList.add(`${phase}-${themeType}`);
        bodyEl.classList.add(phase);

        // Precipitation overlays
        appContainer.classList.remove('rainy', 'snowy', 'stormy', 'cloudy', 'clearish');
        if (themeType === 'rain') appContainer.classList.add('rainy');
        if (themeType === 'snow') appContainer.classList.add('snowy');
        if (themeType === 'thunderstorm') appContainer.classList.add('stormy');
        if (themeType === 'clouds') appContainer.classList.add('cloudy');
        if (themeType === 'clear') appContainer.classList.add('clearish');
    }
